...
class InstallCommand extends AICommand
{
    protected $signature = 'ai:install {--reinstall}';

    protected $description = 'install ai admin';
    ...
    protected function publishConfig(): void
    {
        ...
        try {
            ...
            if (Str::of(PHP_OS)->lower()->contains('dar')) {
                ...
                exec(Application::formatCommandString('vendor:publish --tag=ai-config'));
                exec(Application::formatCommandString('vendor:publish --provider="Laravel\Sanctum\SanctumServiceProvider"'));
                exec(Application::formatCommandString('ai:migrate user'));
                exec(Application::formatCommandString('ai:migrate develop'));
                exec(Application::formatCommandString('migrate'));
                exec(Application::formatCommandString('ai:db:seed user'));
                if ($isInstallPermissionModule) {
                    exec(Application::formatCommandString('ai:migrate permissions'));
                    exec(Application::formatCommandString('ai:db:seed permissions'));
                }
            } else {
                Process::run(Application::formatCommandString('key:generate'))->throw();
                Process::run(Application::formatCommandString('vendor:publish --tag=ai-config'))->throw();
                Process::run(Application::formatCommandString('vendor:publish --provider="Laravel\Sanctum\SanctumServiceProvider"'))->throw();
                Process::run(Application::formatCommandString('ai:migrate user'))->throw();
                Process::run(Application::formatCommandString('ai:migrate develop'))->throw();
                Process::run(Application::formatCommandString('migrate'))->throw();
                Process::run(Application::formatCommandString('ai:db:seed user'))->throw();
                if ($isInstallPermissionModule === '是') {
                    Process::run(Application::formatCommandString('ai:migrate permissions'))->throw();
                    Process::run(Application::formatCommandString('ai:db:seed permissions'))->throw();
                }
            }
    ...
    protected function addPsr4Autoload(): void
    {
        ...
        $composerJson['autoload']['psr-4'][AIAdmin::getModuleRootNamespace()] = str_replace('\\', '/', config('ai.module.root'));
    ...
    protected function support(): void
    {
        ...
        if (in_array(strtolower($answer), ['yes', 'y'])) {
            if (PHP_OS_FAMILY == 'Darwin') {
                exec('open https://github.com/azure-dragon-ai/ai-admin');
            }
            if (PHP_OS_FAMILY == 'Windows') {
                exec('start https://github.com/azure-dragon-ai/ai-admin');
            }
            if (PHP_OS_FAMILY == 'Linux') {
                exec('xdg-open https://github.com/azure-dragon-ai/ai-admin');
            }
        }

        $this->info('支 持: https://github.com/azure-dragon-ai/ai-admin');
        $this->info('文 档: https://github.com/azure-dragon-ai/ai-admin');
        $this->info('官 网: https://github.com/azure-dragon-ai/ai-admin');
    }
